...
func TestHandleParts(t *testing.T) {
	...
	inStock := newPart(t, db, "BRK-001", "Brake pad set", 42500, 12)
	none := newPart(t, db, "OIL-005", "Oil filter", 8000, 0)
...
func TestHandleCreatePart(t *testing.T) {
	...
	t.Run("adds the part with the price in millimes", func(t *testing.T) {
		...
		if len(parts) != 1 {
			t.Fatalf("catalogue has %d parts, want 1", len(parts))
		}
		t.Cleanup(func() {
			if err := db.DeletePart(context.Background(), parts[0].ID); err != nil {
				t.Errorf("cleanup failed: %v", err)
			}
		})

		// The point of the whole millimes decision: 42.500 dinars is stored exactly.
		if parts[0].PriceMillimes != 42500 {
			t.Errorf("PriceMillimes = %d, want 42500", parts[0].PriceMillimes)
		}
	})

	t.Run("rejects a duplicate reference as a field error", func(t *testing.T) {
		existing := newPart(t, db, "BRK-888", "Brake pad set", 30000, 3)
...
func TestHandleEditAndDeletePart(t *testing.T) {
	...
	t.Run("the edit form round-trips the price", func(t *testing.T) {
		part := newPart(t, db, "OIL-111", "Oil filter", 8290, 4)
	...
	t.Run("saves the changes", func(t *testing.T) {
		part := newPart(t, db, "OIL-222", "Oil filter", 8000, 4)
